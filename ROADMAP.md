...
- [x] G8 enforced in CI — `dbt build` runs on every push against synthetic fixtures generated by `scripts/make_ci_fixtures.py`, gating 117 of 121 data tests. The four that pin real-world counts carry the `real_data` tag and are excluded, which is the honest boundary of what CI proves. Fixtures are invented rather than sampled, because a public repository is publication under `LEGAL.md`
