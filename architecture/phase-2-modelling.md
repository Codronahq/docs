...
- **Item evidence density is substantially a function of publication date, which compounds the covariate problem above.** Measured 17 Aug 2026: the 447 bank problems first attempted in 2026 are 3.80% of the bank and carry **18.31% of its responses**, at 4,430 responses per problem against 781 — a **5.67×** ratio. This is `activeOnly=true` in per-item form. A problem published inside the collection window is attempted by the whole active cohort at once, while a 2013 problem was attempted by 2013's users, of whom only the still-active remainder is in the corpus. So older items are thinner and shrink harder toward a prior whose second covariate already carries publication date — two independent date effects pointing the same way. They are independent: `solved_count` comes from Codeforces' own population-wide snapshot and is unaffected by our sampling, while the response counts here are entirely a product of it. Any statement about fitted difficulty in the older bands carries both.
...
- **Held-out split is temporal, the cutoff is 2026-01-01, and it partitions the held-out period across three gates.** A random split leaks: a user's later submissions inform their earlier ones. Measured 17 Aug 2026 over the 4,373,038 held-out bank responses, a test response is scoreable by G1 only if both its user and its item carry fitted parameters: **41.51% qualify (1,815,276 responses, baseline 0.241004)**, 36.80% land on items with no training history and belong to G3, 13.2024% on users with none and belong to G2, and 8.4846% to both — shares to four decimals so a reader can perform the sum, and measured rather than derived by subtracting rounded numbers, which is how this line read 8.49% until 17 Aug 2026. G1's 41.51% is its scope rather than a shortfall, and every remaining response is routed rather than dropped. A later cutoff is better on **both** axes, which is not the expected trade-off: 2023-01-01 holds out 90.92% of the bank and scores 4.43% of it, 2024-01-01 scores 9.15%, 2025-01-01 scores 20.10%. Moving the cutoff moves the baseline with it, so it is a stated parameter rather than a fit-time choice. Full derivation in `quality/eval-gates.md` G1.
